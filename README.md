...
当前默认配置：
- Planner：OpenCode `ownplan/deepseekv4pro`，负责规划和任务拆分，降低 Codex 额度消耗
- Developer：Claude，负责代码实现
- Auditor / Final Auditor：Codex，保留审计和最终复核安全门
- Dashboard：中文实时控制台，展示阶段进度、Agent 状态、下一步、运行产物和实时输出
...
- **OpenCode Planner**：默认 `review-loop.yaml` 使用 OpenCode 作为规划者，并带心跳输出，长时间规划时 dashboard 不会像卡死。
- **Claude Developer**：开发阶段继续使用 Claude，并保留代理环境清理和心跳。
- **Codex 审核门**：Auditor 和 Final Auditor 仍使用 Codex，不把强审核切到低成本模型。
- **并行任务图**：Planner 可拆分并行 task，系统按 DAG/wave 执行，支持独立 worktree。
- **中文 Dashboard**：浏览器页面已从事件表格升级为控制台布局，包含主状态卡、阶段进度条、Agent 状态、下一步提示、运行产物、时间线和实时输出。
- **阻塞可见性**：`task.blocked`、`integration.blocked`、`provider.failure` 会在页面上明确显示为阻塞/失败，而不是继续显示“正在开发”。
- **恢复和重试**：支持 `resume`、`retry`、stale lock 自动恢复、cancel 请求和历史 run 浏览。
...
打开后能看到：
- 主状态卡：当前阶段、角色、模型、轮次、耗时、最近事件
- 阶段进度条：初始化 → 规划 → 开发 → 验证 → 审计 → 最终复核 → 完成
- Agent 状态：Planner / Developer / Auditor / Final Auditor 的 provider、状态和耗时
- 下一步提示：告诉用户现在是否需要等待、处理阻塞、resume 或查看日志
- 执行时间线：role/task/wave/integration/audit/provider failure 等事件
- 实时 agent 输出：`<thinking>` 已过滤，只显示可见进度
- 运行产物链接：transcript、plan、GOAL、audit-report、verification log、state 等
- 历史运行切换器
- 取消按钮
...
```yaml
agents:
  planner:        # 默认 opencode/ownplan/deepseekv4pro，负责拆任务
  developer:      # 默认 claude，负责写代码
  auditor:        # 默认 codex，审计每轮 diff
  final_auditor:  # 默认 codex，审计合并后整体
